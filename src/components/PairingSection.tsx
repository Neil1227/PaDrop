...
export const PairingSection: React.FC<PairingSectionProps> = ({
  ...
  // Receiver is Host (generates QR), Sender is Guest (scans QR & Radar)
  ...
  useEffect(() => {
    setActiveTab(isHost ? 'receive' : 'send');
  }, [isHost]);
  ...
  // Receiver generates a pairUrl pointing to Sender Mode (?mode=send)
  const pairUrl = `${effectiveOrigin}/?room=${roomId}&mode=send`;
  ...
  const handleTabSwitch = (tab: 'send' | 'receive') => {
    setActiveTab(tab);
    if (onSwitchRole) {
      // Invert: 'receive' is host (creates new room), 'send' is guest (scanner)
      onSwitchRole(tab === 'receive' ? 'host' : 'receive');
    }
  };
  ...
  return (
    <section className="w-full flex flex-col gap-4 transition-all">
      {/* STATE A: CONNECTED STATE (Streamlined Status Bar with Disconnect Action) */}
      {isConnected ? (
        <div className="w-full bg-surface border border-emerald-500/40 rounded-2xl p-4 sm:p-5 shadow-surface-elevated flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 animate-fadeIn">
          <div className="flex items-center gap-3.5 w-full sm:w-auto">
            {/* Animated Glowing Beacon */}
            <div className="w-10 h-10 sm:w-11 sm:h-11 rounded-2xl bg-emerald-500/15 border border-emerald-500/30 flex items-center justify-center shrink-0 shadow-inner">
              <span className="relative flex h-3 w-3">
                <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-emerald-400 opacity-75"></span>
                <span className="relative inline-flex rounded-full h-3 w-3 bg-emerald-400 shadow-[0_0_10px_#10B981]"></span>
              </span>
            </div>
            ...
            <div className="flex flex-col text-left">
              <div className="flex items-center gap-2 flex-wrap">
                <h3 className="text-sm sm:text-base font-bold text-white tracking-tight">
                  Connected to {isHost ? 'Sender' : 'Receiver'}
                </h3>
                <span className={`text-[9px] font-mono font-bold px-1.5 py-0.5 rounded shadow-xs tracking-wider uppercase ${
                  isHost ? 'bg-cobalt text-white' : 'bg-signal-gradient text-white'
                }`}>
              ...
              <p className="text-xs text-slate-300 mt-0.5 leading-relaxed">
                Direct WebRTC peer channel is active • Real-time clipboard sync & 64 KB file drop enabled
              </p>
            </div>
          </div>
          ...
          {onDisconnect && (
            <button
              id="disconnect-peer-btn"
              type="button"
              onClick={onDisconnect}
              className="w-full sm:w-auto min-h-[42px] px-4 py-2 rounded-xl bg-rose-500/10 hover:bg-rose-500/20 active:scale-95 border border-rose-500/30 text-rose-300 hover:text-rose-100 text-xs sm:text-sm font-semibold transition-all flex items-center justify-center gap-2 shrink-0 shadow-sm"
            >
              <Unlink className="w-4 h-4 text-rose-400 shrink-0" />
              <span>Disconnect</span>
            </button>
          )}
        </div>
      ) : (
        /* STATE B: DISCONNECTED / PAIRING STATE (Role Selection Tabs & Discovery Cards) */
        <>
          {/* Role Selection Tabs */}
          ...
          {/* VIEW 1: SENDER VIEW (Camera Scanner, Radar & Manual Room Entry) */}
          {activeTab === 'send' && (
            <div className="w-full flex flex-col gap-5 animate-fadeIn">
              {/* Hero In-App Camera Scanner Action Card */}
              <div className="w-full bg-surface border border-signalStart/40 rounded-2xl p-4 sm:p-5 shadow-surface-elevated flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                <div className="flex items-center gap-3.5 w-full sm:w-auto">
                  <div className="w-11 h-11 sm:w-12 sm:h-12 rounded-2xl bg-signalStart/20 border border-signalStart/40 text-signalStart flex items-center justify-center shrink-0 shadow-inner">
                  ...
                  <div className="flex flex-col text-left">
                    ...
                    <p className="text-xs text-slate-300 mt-0.5 leading-relaxed">
                      Scan the QR code displayed on the receiving device to send files directly.
                    </p>
                  </div>
                </div>
                  ...
                  <div className="flex flex-col gap-3">
                    ...
                    <form onSubmit={handleManualJoin} className="flex flex-col gap-3 mt-1">
                      ...
                      <button
                        ...
                        className="w-full min-h-[44px] flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-signal-gradient hover:opacity-95 disabled:opacity-40 disabled:cursor-not-allowed text-xs font-semibold text-white transition-all shadow-sm active:scale-95"
                      >
          ...
          {/* VIEW 2: RECEIVER VIEW (Host QR Code & Sharing Link) */}
          {activeTab === 'receive' && (
              ...
              <div className="flex flex-col md:flex-row items-center justify-between gap-6">
                ...
                <div className="flex flex-col sm:flex-row items-center gap-5 w-full md:w-auto">
                  <div className="relative p-3.5 rounded-2xl bg-white shadow-xl flex items-center justify-center shrink-0 group">
                    ...
                    />
                    <div className="absolute inset-0 rounded-2xl border-2 border-transparent group-hover:border-cobaltLight/40 transition-colors pointer-events-none" />
                  </div>
                  ...
                  <div className="flex flex-col gap-2 text-center sm:text-left max-w-md">
                    ...
                    <p className="text-xs text-slate-300 leading-relaxed">
                      Show this QR code to the sender or make this device visible on the Radar to receive files and live clipboard text.
                    </p>
                    ...
                    <div className="flex flex-wrap items-center justify-center sm:justify-start gap-2.5 mt-1">
                      <button
                        ...
                        className="min-h-[44px] inline-flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-bold text-white bg-cobalt hover:bg-cobaltLight active:scale-95 transition-all shadow-[0_0_16px_rgba(0,71,171,0.45)]"
                      >
                        {copiedLink ? (
                          <>
                      ...
                      <button
                        ...
                        title={isDiscoverable ? 'Device is discoverable to nearby senders' : 'Device is hidden from nearby discovery'}
                      ...
                      <div className="mt-1.5 text-left">
                        {!showLanConfig ? (
                          <button
                            ...
                            className="text-[11px] text-slate-400 hover:text-cobaltLight underline decoration-dotted decoration-slate-500 transition-colors flex items-center gap-1"
                          >
                            <span>📱</span>
                            <span>
                              {lanIp
                          ...
                          <div className="flex flex-col gap-1.5 p-2.5 rounded-xl bg-canvas border border-surfaceBorder mt-1 max-w-sm">
                            <span className="text-[11px] text-slate-300 font-medium">
                              Enter your PC's Wi-Fi IP so sender's phone camera can reach localhost:
                            </span>
                            <div className="flex items-center gap-1.5">
                              <input
                                ...
                                className="min-h-[38px] px-2.5 py-1 rounded-lg bg-surface border border-surfaceBorder text-white text-xs font-mono flex-1 focus:outline-none focus:border-cobaltLight"
                              />
                              <button
                                type="button"
                                onClick={() => {
                                ...
                                className="min-h-[38px] px-3 py-1 rounded-lg bg-cobalt text-white text-xs font-semibold shadow-sm hover:opacity-90"
                              >
                                Save
                              </button>
                              {lanIp && (
